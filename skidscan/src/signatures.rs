...
impl Signature {
    ...
    pub unsafe fn scan_module<S: AsRef<str>>(
        &self,
        module: S,
    ) -> Result<*mut u8, ModuleSigScanError> {
        let scanner =
            Scanner::for_module(module.as_ref()).ok_or(ModuleSigScanError::InvalidModule)?;
        scanner.find(&*self)
    }
}
                ...
                (2, _) => {
                    ...
                    signature.push_byte(
                        u8::from_str_radix(byte, 16)
                            .map_err(|_| SignatureParseError::InvalidByte)?,
                    );
                }
