...
fn test_ptr_scan() {
    unsafe {
        ...
        assert!(
            Signature::from_str("55 8B EC ?? ?? F8 83")
                .unwrap()
...
fn test_ptr_scan_sub() {
    unsafe {
        ...
        assert!(
            Signature::from_str("24 8B 45 ?? ?? 44 24")
                .unwrap()
                .scan_ptr(bytes.as_ptr(), bytes.as_ptr().add(bytes.len()))
                .is_some()
        );
...
fn test_ptr_scan_sub_rewind() {
    unsafe {
        ...
        assert_eq!(ptr, bytes.as_ptr().add(19));
    }
}
...
fn test_ptr_scan_sub_fail() {
    unsafe {
        ...
        assert!(
            Signature::from_str("24 8B 45 ?? ?? 44 24 69")
                .unwrap()
                .scan_ptr(bytes.as_ptr(), bytes.as_ptr().add(bytes.len()))
                .is_none()
        );
...
fn test_ptr_scan_sub_oob() {
    unsafe {
        ...
        assert!(
            Signature::from_str("8B")
                .unwrap()
                .scan_ptr(bytes.as_ptr(), bytes.as_ptr().add(1))
                .is_none()
        );
        assert!(
            Signature::from_str("55")
                .unwrap()
                .scan_ptr(bytes.as_ptr(), bytes.as_ptr().add(1))
                .is_some()
        );
...
fn test_ptr_scan_fail() {
    unsafe {
        ...
        assert!(
            Signature::from_str("55 8B EC ?? ?? FF FF")
                .unwrap()
                .scan_ptr(bytes.as_ptr(), bytes.as_ptr().add(bytes.len()))
                .is_none()
        );
